...
# https://psl.noaa.gov/data/correlation/ao.data
 ...
 colnames(AOI)<-c("year", seq(1,12,1))

# combine AO and ONI into time series
 ONI<-melt(ONI, measure.vars = 2:13)
 ONI$moDate<-as.Date(paste0(ONI$variable,"-01-",ONI$year),format = "%m-%d-%Y")
 AOI<-melt(AOI, measure.vars = 2:13)
 AOI$moDate<-as.Date(paste0(AOI$variable,"-01-",AOI$year),format = "%m-%d-%Y")
 # combine on date
 climInd<-merge(ONI,AOI, by="moDate")
 climInd<-climInd[,c(-5,-6)]
 colnames(climInd)[c(2,4,5)]<-c("year","ONI","AO")

 save(climInd, file = "~/RProjects/WinterSummerPrecip/climInd.RData")
